...
__aicore__ inline void Init(GM_ADDR x, GM_ADDR quantized_weight, GM_ADDR weight_scale, GM_ADDR bias, GM_ADDR x_scale,
                            GM_ADDR x_offset, GM_ADDR smooth_scale,
                            GM_ADDR y, GM_ADDR usrWorkspace, const QuantMatmulDequantTilingData* __restrict qmmTiling)
{
 ...
 protected:
  ...
  __aicore__ inline void ProcessX() {
    const bool decodeShape = tilingData->originE == DECODE_EXPERTS && tilingData->originM == DECODE_ROUTED_ROWS &&
        ((tilingData->originK == DECODE_HIDDEN_SIZE && tilingData->originN == DECODE_GATE_UP_SIZE) ||
         (tilingData->originK == DECODE_INTERMEDIATE_SIZE && tilingData->originN == DECODE_HIDDEN_SIZE));
    if (decodeShape && realM <= GEMV_THRESHOLD && tilingData->perToken &&
        !tilingData->dynamicQuant && !tilingData->smoothScale && !isWScaleInt64) {
      ProcessXBatchedDecode();
      return;
    }
  ...
  __aicore__ inline void ProcessXBatchedDecode() {
    // At most 8 x 2048 values. These non-overlapping slices consume < 113 KiB
    // of the existing 256 KiB UB and are dead before ProcessY reuses the UB.
    const uint32_t width = tilingData->originK;
    const uint32_t count = realM * width;
    LocalTensor<half> batchHalf = ubXHalfGemv;
    LocalTensor<float> batchFloat = batchHalf[count].ReinterpretCast<float>();
    LocalTensor<int8_t> batchInt8 = batchFloat[count].ReinterpretCast<int8_t>();
    LocalTensor<float> batchScales = batchInt8[count].ReinterpretCast<float>();
    DataCopy<half>(batchHalf, xGm, count);
    DataCopy<float>(batchScales, xScaleGm,
                    (realM + FLOAT_PERBLOCK - 1) / FLOAT_PERBLOCK * FLOAT_PERBLOCK);
    SetFlag<HardEvent::MTE2_V>(eventIdMTE2ToV[0]);
    SetFlag<HardEvent::MTE2_S>(eventIdMTE2ToS[0]);
    WaitFlag<HardEvent::MTE2_S>(eventIdMTE2ToS[0]);
    for (uint32_t row = 0; row < realM; ++row) {
      pertokenScaleDequant[row] = batchScales.GetValue(row);
    }
    WaitFlag<HardEvent::MTE2_V>(eventIdMTE2ToV[0]);
    UnaryRepeatParams h2f, f2i, i2h, h2i8, unary;
    h2f.srcRepStride = HALF_DEFAULT_REPEAT_STRIDE;
    i2h.dstRepStride = HALF_DEFAULT_REPEAT_STRIDE;
    h2i8.dstRepStride = HALF_DEFAULT_REPEAT_STRIDE;
    SetMaskCount();
    SetVectorMask<half, MaskMode::COUNTER>(count);
    Cast<float, half, false>(batchFloat, batchHalf, RoundMode::CAST_NONE,
                            MASK_PLACEHOLDER, 1, h2f);
    PipeBarrier<PIPE_V>();
    SetVectorMask<float, MaskMode::COUNTER>(width);
    for (uint32_t row = 0; row < realM; ++row) {
      Muls<float, false>(batchFloat[row * width], batchFloat[row * width],
                         FLOAT_1 / pertokenScaleDequant[row], MASK_PLACEHOLDER, 1, unary);
    }
    PipeBarrier<PIPE_V>();
    SetVectorMask<float, MaskMode::COUNTER>(count);
    Cast<int32_t, float, false>(batchFloat.ReinterpretCast<int32_t>(), batchFloat,
                               RoundMode::CAST_RINT, MASK_PLACEHOLDER, 1, f2i);
    PipeBarrier<PIPE_V>();
    SetDeqScale((half)FLOAT_1);
    Cast<half, int32_t, false>(batchHalf, batchFloat.ReinterpretCast<int32_t>(),
                              RoundMode::CAST_NONE, MASK_PLACEHOLDER, 1, i2h);
    PipeBarrier<PIPE_V>();
    Cast<int8_t, half, false>(batchInt8, batchHalf, RoundMode::CAST_ROUND,
                             MASK_PLACEHOLDER, 1, h2i8);
    SetFlag<HardEvent::V_MTE3>(eventIdVToMTE3[0]);
    WaitFlag<HardEvent::V_MTE3>(eventIdVToMTE3[0]);
    DataCopyParams copy;
    copy.blockCount = realM;
    copy.blockLen = width / INT8_PERBLOCK;
    copy.srcStride = 0;
    copy.dstStride = (tilingData->originKAligned512 - width) / INT8_PERBLOCK;
    DataCopy<int8_t>(l1XGemv, batchInt8, copy);
    SetFlag<HardEvent::MTE3_MTE2>(eventIdMTE3ToMTE2[0]);
    WaitFlag<HardEvent::MTE3_MTE2>(eventIdMTE3ToMTE2[0]);
    SetFlag<HardEvent::MTE3_MTE1>(eventIdMTE3ToMTE1[0]);
    WaitFlag<HardEvent::MTE3_MTE1>(eventIdMTE3ToMTE1[0]);
  ...
  static constexpr uint32_t DECODE_INTERMEDIATE_SIZE = 256;
