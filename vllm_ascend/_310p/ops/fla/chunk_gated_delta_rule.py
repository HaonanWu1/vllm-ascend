...
CHUNK_SIZE = GDN_PREFILL_CHUNK_SIZE
SUPPORTED_COMPUTE_WY_HEAD_DIMS = (64, 128)
...
def _can_use_npu_compute_wy(
    ...
    if q.shape[2] <= 0 or v.shape[2] <= 0:
        return False
    # Must mirror the binding and tiler. The Cube path uses static 64-wide
    # slices, with one slice for 64 and two slices for 128.
    if q.shape[3] not in SUPPORTED_COMPUTE_WY_HEAD_DIMS or v.shape[3] not in SUPPORTED_COMPUTE_WY_HEAD_DIMS:
        return False
    # Mirror host tiling: B<=32, Hv<=64.
